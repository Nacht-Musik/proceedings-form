function init(){
  // 会議カテゴリーの選択
  $('#meeting-category-sel').change(function() {
    // 選択オプションのvalueを取得
  ...
  $('.add-btn').click(function(){
    // 追加する人の氏名を取得
    var name = $('#name-sel option:selected').text();

    // 追加する人の役割を取得
    var role = $('#role-sel option:selected').text();
    ...
    if (name === "" && role === "") {
      ...
      $('#alert-disp').text('');
      // 該当箇所に選択中の氏名、役割を代入。
      $(elm).find('.person-name').html(name);
      $(elm).find('.person-role').html(role);
